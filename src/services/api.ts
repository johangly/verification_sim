import { PhoneNumber, CreatePhoneNumberRequest, CreatePhoneNumberByFileRequest, UpdatePhoneNumberRequest } from '../types/phoneNumber';
...
class PhoneNumberService {
  ...
  async getAllPhoneNumbers(elementsPerPage?: number, range?: { start: Date | undefined; end: Date | undefined }): Promise<PhoneNumber[]> {
    return this.request<PhoneNumber[]>(`/phonenumbers?amount=${elementsPerPage}&start=${range?.start?.toISOString()}&end=${range?.end?.toISOString()}`);
  }
  ...
  async createPhoneNumberByFile(data: CreatePhoneNumberByFileRequest): Promise<PhoneNumber> {
