...
export default function ConcentratedForm(
    { concentrated, setConcentrated, idUpdating, CreateConcentrated, UpdateConcentrated }: ConcentratedPageProps
) {
    const inputs = [
        {
            label: 'Nombre',
        ...
        },
        {
            ...
            onChange: (e: React.ChangeEvent<HTMLInputElement>) =>
                setConcentrated({ ...concentrated, isActive: e.target.checked }),
        }
    ]
    return (
                    ...
                    {inputs.map((input, index) => (
                        ...
                        >
                            <label className="mb-2 font-medium text-gray-700 dark:text-gray-300 mt-4">
                                {input.label === 'Nombre' && <User className="inline-block mr-2 w-4 h-4 text-gray-400 dark:text-gray-500" />}
                                {input.label === 'Descripción' && <Info className="inline-block mr-2 w-4 h-4 text-gray-400 dark:text-gray-500" />}
                                {input.label === 'Estado' && <Check className="inline-block mr-2 w-4 h-4 text-gray-400 dark:text-gray-500" />}
                                {input.label}
                            </label>
                            {input.type === 'checkbox' ? (
                                <input
