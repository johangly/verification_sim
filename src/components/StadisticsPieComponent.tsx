...
export default function StadisticsPieComponent({title,subtitle,data,labelPassed,footerText,subFooterText}:StadisticsPieComponentProps){
                    ...
                    <div className='flex flex-col gap-2 mb-2'>
                        {data.map((item, index) => (
                            <div key={index} className='flex items-center gap-2'>
                                ...
                                <span className='text-sm font-bold'>{item.name.toUpperCase()} : {item.value}</span>
                            </div>
                        ))}
                    </div>
                <ChartContainer
                    config={chartConfig }
                    className="mx-auto w-auto max-w-[600px] max-h-[500px]"
                >
                    <PieChart
