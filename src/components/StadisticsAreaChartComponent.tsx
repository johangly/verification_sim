...
function groupByDay<T extends { date: string }>(
    ...
    data.forEach(item => {
        ...
        if (!grouped[day]) {
            grouped[day] = { date: day, ...Object.fromEntries(keys.map(key => [key as string, 0])) };
        }
        keys.forEach(key => {
...
export default function StadisticsAreaChartComponent({
    ...
    const groupedData = useMemo(() => groupByDay(data, areaKeys), [data, areaKeys]); const filteredData = useMemo(() => {
    ...
    return (
        <Card className="pt-0 bg-gray-800 w-full flex-1 min-w-[750px]">
