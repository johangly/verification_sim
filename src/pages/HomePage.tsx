...
import {PhoneNumber, PhoneNumberPaginated} from '../types/phoneNumber';
...
export const HomePage: React.FC = () => {
    ...
    const [infoPagination, setInfoPagination] = useState<PhoneNumberPaginated['pagination']>({
        currentPage: 0,
        pageSize: 0,
        totalItems: 0,
        totalPages: 0
     ...
     */
    const loadPhoneNumbers = useCallback(async (range?: { start: Date | undefined; end: Date | undefined }) => {

        try {
            setIsLoading(true);
            const data:PhoneNumberPaginated = await phoneNumberService.getAllPhoneNumbers(
                page,
                rowPerPage,
                range
            );
            ...
            setFilteredPhoneNumbers(data.data);
        } catch (error) {
     ...
     */
    const handleUpdatePhoneNumber = async (data: {
        phoneNumber: string,
    ...
    const tableData = filteredPhoneNumbers.map(phone => ({
        id: phone.id,
        phoneNumber: phone.phoneNumber,
        status: phone.status,
        createdAt: new Date(phone.createdAt).toLocaleDateString('es-ES', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
        }),
        createdAtRaw: phone.createdAt
    }));
    // Cargar números telefónicos al montar el componente
    useEffect(() => {
        loadPhoneNumbers();
    }, [page, rowPerPage, loadPhoneNumbers]);
            ...
            <div className='max-w-4xl w-full space-y-6'>
                            ...
                            {selectedPhoneNumbers.length > 0 && (
                                <Tooltip delayDuration={500}>
                                    ...
                                    <TooltipContent side='bottom'>
                                        <p className='max-w-xs'>Envia un mensaje de WhatsApp a todos los numeros de
                                            telefono de los clientes seleccionados para validar si el numero es valido
                                            y/o esta en uso, segun la respuesta del cliente se actualizara el
                                            estado.</p>
                                    </TooltipContent>
                                </Tooltip>
                            )}
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <Tooltip delayDuration={500}>
                                <TooltipTrigger disabled={isLoading}>
                                    ...
                                    >
                                        {viewMode === 'grid' ? <List className="w-4 h-4"/> :
                ...
                <AnimatePresence mode="wait">
                    {isLoading ? (
                                ...
                                headers={[
                                    {
                                        ...
                                        key: 'phoneNumber',
                                        cellClassName: 'font-mono text-md text-gray-700 dark:text-gray-300',
                                        render: (value) => <span
                                            className={`px-4 py-2 rounded-full text-md bg-blue-50 text-blue-600 `}>{value}</span>
                                    },
                                    {
                                        icon: SquareCheckBig,
                                        title: 'Estado',
                                        ...
                                        render: (value) => (
                                            <span
                                                className={`w-[120px] text-center inline-block px-2 py-1 rounded-full text-sm ${
                                                    value === 'verificado'
                                    ...
                                    },
                                    {
                                        ...
                                        key: 'createdAt',
                                        sortable: (a, b) => (new Date(a.createdAtRaw).getTime() || 0) - (new Date(b.createdAtRaw).getTime() || 0),
                                        render: (value) => value
                                    },
                                ]}
                                data={tableData}
                        ...
                        >
                            <div
                                className="bg-gray-100 dark:bg-gray-800 rounded-full w-16 h-16 mx-auto mb-4 flex items-center justify-center">
                    ...
                    >
                        <div className='w-[280px] p-4 flex flex-col space-y-3'>
                            <div className='flex flex-col justify-center items-start space-y-2'>
                                <label htmlFor=""
                                       className="text-sm font-medium text-gray-900 dark:text-white opacity-70">Estado</label>
                                <Select value={statusFilter.toString()}
                                        onValueChange={(value) => setStatusFilter(value as PhoneNumberStatus | 'all')}>
                                    <SelectTrigger style={{padding: '.5rem 1rem', height: '42px'}}
                                                   className="w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-[1rem] font-inherit text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        <SelectValue/>
                                    </SelectTrigger>
                                    <SelectContent className='bg-white dark:bg-gray-700'>
                                        <SelectItem
                                            className={twMerge(statusFilter === 'all' && 'bg-blue-600 text-white')}
                                            value="all">Todos los estados</SelectItem>
                                        <SelectItem
                                            className={twMerge(statusFilter === 'verificado' && 'bg-blue-600 text-white')}
                                            value="verificado">Verificado</SelectItem>
                                        <SelectItem
                                            className={twMerge(statusFilter === 'por verificar' && 'bg-blue-600 text-white')}
                                            value="por verificar">Por Verificar</SelectItem>
                                        <SelectItem
                                            className={twMerge(statusFilter === 'no verificado' && 'bg-blue-600 text-white')}
                                            value="no verificado">No Verificado</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <div className='flex flex-col justify-center items-start space-y-2'>
                                <label htmlFor=""
                                       className="text-sm font-medium text-gray-900 dark:text-white opacity-70">Clientes
                                    por pagina</label>
                                <Select value={elementsPerPage.toString()}
                                        onValueChange={(value) => setElementsPerPage(Number(value))}>
                                    <SelectTrigger style={{padding: '.5rem 1rem', height: '42px'}}
                                                   className="w-full border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-[1rem] font-inherit text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        <SelectValue/>
                                    </SelectTrigger>
                                    <SelectContent className='bg-white dark:bg-gray-700'>
                                        <SelectItem
                                            className={twMerge(elementsPerPage === 50 && 'bg-blue-600 text-white')}
                                            value="50">50</SelectItem>
                                        <SelectItem
                                            className={twMerge(elementsPerPage === 100 && 'bg-blue-600 text-white')}
                                            value="100">100</SelectItem>
                                        <SelectItem
                                            className={twMerge(elementsPerPage === 200 && 'bg-blue-600 text-white')}
                                            value="200">200</SelectItem>
                                        <SelectItem
                                            className={twMerge(elementsPerPage === 500 && 'bg-blue-600 text-white')}
                                            value="500">500</SelectItem>
                                        <SelectItem
                                            className={twMerge(elementsPerPage === 1000 && 'bg-blue-600 text-white')}
                                            value="1000">1000</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            <label htmlFor="first_date"
                                   className="text-sm font-medium text-gray-900 dark:text-white opacity-70">Fecha de
                                creacion</label>
                            <div className="flex flex-col justify-center space-y-2 ml-2">
                                <label htmlFor="first_date"
                                       className="text-sm font-medium text-gray-900 dark:text-white opacity-70">Inicio</label>
                                <div className="flex items-center w-full gap-2">
                                    <Popover open={openStartCreationCalendar}
                                             onOpenChange={setOpenStartCreationCalendar}>
                            ...
                            <div className="flex flex-col justify-center space-y-2 ml-2">
                                <label htmlFor="first_date"
                                       className="text-sm font-medium text-gray-900 dark:text-white opacity-70">Final</label>
                                <div className="flex items-center w-full gap-2">
