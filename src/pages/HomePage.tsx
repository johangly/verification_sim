...
export const HomePage: React.FC = () => {
  ...
  const [openStartCreationCalendar, setOpenStartCreationCalendar] =
    useState(false);
  ...
  const [infoPagination, setInfoPagination] = useState<
    PhoneNumberPaginated["pagination"]
  >({
    currentPage: 0,
    pageSize: 0,
    totalItems: 0,
  ...
  const [isStartCampaignDialogOpen, setIsStartCampaignDialogOpen] =
    useState(false);
  ...
  const handleStartCampaign = async () => {
    ...
    const phoneNumbersToSend = selectedPhoneNumbers.map(
    ...
    try {
      ...
      setPhoneNumbers((prev) => {
        ...
        return prev.map((phone: PhoneNumber) => {
          const updatedPhone = updatedNumbers.find(
            (up: PhoneNumber) => up.id === phone.id
          );
  ...
  const loadPhoneNumbers = useCallback(
    async (range?: { start: Date | undefined; end: Date | undefined }) => {
      try {
        setIsLoading(true);
        const data: PhoneNumberPaginated =
          await phoneNumberService.getAllPhoneNumbers(page, rowPerPage, range);
   ...
   */
  const handleCreatePhoneNumberByFile = async ({
    file,
    status,
  }: CreatePhoneNumberByFileRequest) => {
    setIsSubmitting(true);
                ...
                <Tooltip delayDuration={500}>
                  ...
                  <TooltipContent side="bottom">
                    <p className="max-w-xs">
                      Envia un mensaje de WhatsApp a todos los numeros de
                      telefono de los clientes seleccionados para validar si el
                      numero es valido y/o esta en uso, segun la respuesta del
                      cliente se actualizara el estado.
                    </p>
                  </TooltipContent>
                </Tooltip>
              )}
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Tooltip delayDuration={500}>
              ...
              <TooltipContent>
                <p>
                  {areAllSelected
                    ? "Deseleccionar todos los clientes mostrados"
                    : "Seleccionar todos los clientes mostrados"}
                </p>
              </TooltipContent>
            </Tooltip>
              ...
              <DataTableComponent
                headers={[
                  ...
                  },
                  {
                    ...
                    key: "createdAt",
                    sortable: (a, b) =>
                      (new Date(a.createdAtRaw).getTime() || 0) -
                      (new Date(b.createdAtRaw).getTime() || 0),
                    render: (value) => value,
                  },
                ]}
                data={tableData}
          ...
          >
            <div className="w-[280px] p-4 flex flex-col space-y-3">
              <div className="flex flex-col justify-center items-start space-y-2">
                <label
                  htmlFor=""
                  className="text-sm font-medium text-gray-900 dark:text-white opacity-70"
                >
                  Estado
                </label>
                <Select
                ...
                >
                  <SelectTrigger
                    style={{ padding: ".5rem 1rem", height: "42px" }}
              ...
              <div className="flex flex-col justify-center items-start space-y-2">
                ...
                >
                  ...
                  <SelectContent className="bg-white dark:bg-gray-700">
                    <SelectItem
                      className={twMerge(
                        elementsPerPage === 50 && "bg-blue-600 text-white"
                      )}
                      value="50"
                    >
                      50
                    ...
                    <SelectItem
                      className={twMerge(
                        elementsPerPage === 100 && "bg-blue-600 text-white"
                      )}
                      value="100"
                    >
                      100
                    ...
                    <SelectItem
                      className={twMerge(
                        elementsPerPage === 200 && "bg-blue-600 text-white"
                      )}
                      value="200"
                    >
                      200
                    ...
                    <SelectItem
                      className={twMerge(
                        elementsPerPage === 500 && "bg-blue-600 text-white"
                      )}
                      value="500"
                    >
                      500
                    ...
                    <SelectItem
                      className={twMerge(
                        elementsPerPage === 1000 && "bg-blue-600 text-white"
                      )}
                      value="1000"
                    >
                      1000
              ...
              <div className="flex flex-col justify-center space-y-2 ml-2">
                ...
                <div className="flex items-center w-full gap-2">
                  <Popover
                    open={openStartCreationCalendar}
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() =>
                      setRange((prev) => ({ ...prev, start: undefined }))
                    }
              ...
              <div className="flex flex-col justify-center space-y-2 ml-2">
                ...
                <div className="flex items-center w-full gap-2">
                  <Popover
                    open={openEndCreationCalendar}
                    onOpenChange={setOpenEndCreationCalendar}
                  >
                    <PopoverTrigger className="w-full">
                      <Button
                        variant="outline"
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() =>
                      setRange((prev) => ({ ...prev, end: undefined }))
                    }
                    className="p-3 bg-red-500/50 text-white dark:bg-red-500/50 dark:text-white border border-gray-300 dark:border-gray-700 hover:bg-red-500/70 dark:hover:bg-red-500/70 rounded-lg transition-colors"
                    title="Limpiar fecha"
                  >
