...
import { Check, LayoutGrid, Plus, Search, Info, Calendar, User, SquareCheckBig } from 'lucide-react';
...
export default function ConcentratedPage(
    ...
    const { allConcentrated, idUpdating, showModal, setShowModal,fetchConcentratedById,concentrated,setConcentrated,CreateConcentrated,UpdateConcentrated} = useConcentrated();
    ...
    return (
        <div className={twMerge("w-full flex justify-center items-start gap-5")}>
            {showModal && (
                <Modal showModal={showModal} title='Registrar Concentrado' setModal={setShowModal}>
                    <ConcentratedForm 
            ...
            <div className='max-w-5xl w-full space-y-6'>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    >
                        ...
                        ><Plus className="w-4 h-4" />Agregar Concentrado</button>
                    </motion.div>
                </div>
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    ...
                    >
                        <DataTableComponent headers={[
                            ...
                            },
                            {
                                ...
                                title: 'Estado',
                                key: 'isActive',
                                render: (value) => value ? <span
                                    className={`px-4 py-2 rounded-full text-md bg-green-50 text-green-600 `}>Activo</span> :
                                    <span
                                        className={`px-4 py-2 rounded-full text-md bg-red-50 text-red-600 `}>Inactivo</span>
                            }
                            ,
                            {
                                icon: Calendar,
