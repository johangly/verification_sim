...
import { CampaignDetailsModal } from "../components/successCampaignDetails";

const VITE_SOCKET_API_BASE = import.meta.env.VITE_SOCKET_API_BASE;
const VITE_SOCKET_API_END = import.meta.env.VITE_SOCKET_API_END;
export default function CampaignsPage() {
  ...
  return (
    <div className={twMerge("w-full flex justify-center items-start gap-5")}>
      <AnimatePresence>
        {createNewCampaign ? (
          <CampaignForm setCreateNewCampaign={setCreateNewCampaign} />
        ) : (
          <CampaignList
            campaigns={allCampaigns}
            setCreateNewCampaign={setCreateNewCampaign}
          />
        )}
      </AnimatePresence>
    </div>
...
function CampaignForm({
  setCreateNewCampaign,
}: {
  setCreateNewCampaign: (value: boolean) => void;
}) {
  // estados
  ...
  const [campaignCreationDetails, setCampaignCreationDetails] = useState({
    success: false,
    message: "",
    campaignId: 0,
    invalidNumbers: [],
  ...
  const procesarDatos = async (data: any[]) => {
    try {
      const response = campaignsService.createFullCampaignWithSockets({
        userId,
        phoneNumbers: data,
      });
  ...
  const handleCreatePhoneNumberByFile = async ({
    ...
    setIsLoading(true);
    try {
      const { phoneNumbersToCreate } = await campaignsService.readClientsByFile(
        { file, status }
      );
      // Verificar duplicados
      const existingNumbers = new Set(phoneNumbers.map((p) => p.phoneNumber));
      const duplicates: string[] = [];
      const uniqueNewNumbers = phoneNumbersToCreate.filter(
        (phone: PhoneNumber) => {
      ...
      if (duplicates.length > 0) {
        duplicates.forEach((number) => {
          toast.error(
            `El cliente con el numero "${number}" ya existe y será omitido`
          );
        });
      }
  ...
  const handleStartCampaign = async () => {
    ...
    try {
      const response = await testService.createFullCampaignTest({
        phoneNumbers: phoneNumbers,
      });
      console.log("respuesta de la creacion de campaigns nueva", response);
      if (response.success === true && response.error === false) {
        setIsCampaignDetailsModalOpen(true);
        setCampaignCreationDetails(response);
      }
  ...
  useEffect(() => {
    const newSocket = io(`${VITE_SOCKET_API_BASE}`, {
      path: `${VITE_SOCKET_API_END}`,
    });
      ...
      >
        <div className="flex items-center justify-center space-x-2">
          <motion.button
            ...
            className="flex items-center text-blue-600 hover:text-blue-800 dark:text-white bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 transition-colors cursor-pointer mr-5"
          >
              ...
              <Tooltip delayDuration={500}>
                ...
                <TooltipContent side="bottom">
                  <p className="max-w-xs">
                    Envia un mensaje de WhatsApp a todos los numeros de telefono
                    de los clientes seleccionados para validar si el numero es
                    valido y/o esta en uso, segun la respuesta del cliente se
        ...
        <StartCampaignDialog
          ...
          onConfirm={async () => {
            try {
              await procesarDatos(phoneNumbers);
            } catch (error) {
        ...
        <CampaignDetailsModal
          ...
          onClose={() => setIsCampaignDetailsModalOpen(false)}
          onConfirm={() => {
            // setIsCampaignDetailsModalOpen(false);
            console.log("mostrar datos de campaign recien creada");
          }}
          isLoading={isLoading}
          data={campaignCreationDetails}
        />
      </motion.div>
      <AnimatePresence>
        {sendingCreateCampaign && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50 bg-gray-100 dark:bg-gray-800 flex flex-col items-center justify-center space-y-8"
          >
            <div className="max-h-[200px] max-w-[200px] flex items-center justify-center">
            ...
            >
              <h1 className="text-2xl font-regular text-gray-900 dark:text-gray-100 mb-5 text-center">
                {processStatus.status === "completed"
                  ? "Proceso completado!"
                  : processStatus.status === "error"
                  ? "Error en el proceso"
              ...
              <div className="w-full max-w-[300px] space-y-3 mx-auto">
                <div className="flex justify-center text-center text-md text-gray-600 dark:text-gray-400">
