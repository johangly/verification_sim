...
import { X, Check, CheckCheck, CircleX, MailCheck } from "lucide-react";
    ...
    <div className="w-full flex justify-center items-start gap-5">
      <div className="max-w-7xl w-full space-y-6">
        <div>
          ...
          >
            <div className="flex justify-start items-start gap-8">
              <div className="flex flex-col w-full max-w-[250px]">
                <p className="font-semibold mb-2">Seleccionar Campaña</p>
                <Select
                  value={campaignSelectedToSeeStatistics.toString()}
                  onValueChange={(value) =>
                    setCampaignSelectedToSeeStatistics(parseInt(value))
                  }
                >
                  <SelectTrigger
                    style={{
                  ...
                  <SelectContent className="bg-white dark:bg-gray-700 overflow-y-auto max-h-[300px]">
                    {allCampaigns
                      .slice()
                      .sort(
                        (a, b) =>
                      ...
                      .map((campaign) => (
                        <SelectItem
                          className={twMerge(
                            campaignSelectedToSeeStatistics === campaign.id &&
              ...
              <div className="w-fit">
                <h2 className="text-blue-600 font-bold">ULTIMA CAMPAÑA:</h2>
                <p>
                  {allCampaigns.length > 0
                    ? `${new Date(
                        allCampaigns[allCampaigns.length - 1].createdAt
                      ).toLocaleDateString()}`
                    : "No hay campañas disponibles"}
                </p>
              </div>
            </div>
            <div className="w-full grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6 my-6 bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-6 ">
            <AnimatePresence>
              {campaignMessagesStadistics &&
                ...
                campaignMessagesStadistics.messageStats.map((stat, index) => (
                  <motion.div
                    ...
                    className={twMerge(
                      ...
                      stat.isTotal
                        ? "justify-center border-blue-200 bg-blue-600/50 dark:bg-blue-900 dark:bg-opacity-10 dark:border-blue-800"
                        : `${stat.color.replace(
                            "text",
                            "border"
                  ...
                  >
                    {stat.isTotal ? (
                      ...
                      <>
                        <div
                          className={`mb-2 p-2 rounded-full ${stat.color} bg-opacity-10`}
                        >
                          {stat.icon}
                        </div>
                        <span
                          className={`text-2xl font-bold text-center ${stat.color}`}
                        >
                          {stat.value}
                        </span>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700 mt-2">
                          <div
                            className={`h-1.5 rounded-full ${stat.color.replace(
                              "text",
                              "bg"
                            )}`}
                ...
                <div className="flex gap-6 w-full flex-col lg:flex-row">
                  <div className="w-full flex-1 w-full">
                    <StadisticsPieComponent
                      title={`Estadisticas de la campaña #${campaignSelectedToSeeStatistics}`}
                  ...
                  <div className="w-full flex-1 w-full">
                    <StadisticsPieComponent
                      title={`Estadisticas de la campaña #${campaignSelectedToSeeStatistics}`}
                      subtitle={"Respuestas a los mensajes"}
                      data={dataToShowInGraphMessage}
                      labelPassed={"Si"}
                      footerText={"SI / NO"}
                      subFooterText={"Detalle de respuestas a los mensajes"}
                    />
                  </div>
                </div>
          ...
          {isLoading ? (
            ...
            generalStatistics && (
              <div className="flex flex-col w-full gap-6 items-start">
                ...
                <div className="flex gap-6 flex-wrap-reverse w-full">
                  ...
                  <StadisticsAreaChartComponent
                    ...
                    description={
                      "Datos sobre telefonos verificados y no verificados en rango de tiempo"
                    }
